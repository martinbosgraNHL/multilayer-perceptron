...
class MLP:
    ...
    def __init__(self, alpha: float, batch_size: int, node_size: List[int], num_classes: int, num_features: int) -> None:
    ...
    def train(self, num_epochs: int, log_path: str, train_data: List[ndarray], train_size: int, test_data: List[ndarray], test_size: int, result_path: str) -> None:
        ...
        ts = str(time.strftime('%Y_%m_%d_%H_%M_%S'))

        train_writer = tf.summary.FileWriter(os.path.join(log_path, ts + '-training'), graph=tf.get_default_graph())
        test_writer = tf.summary.FileWriter(os.path.join(log_path, ts + '-test'), graph=tf.get_default_graph())

        with tf.Session() as sess:
            sess.run(init_op)

            try:
                steps = range(num_epochs * train_size // self.batch_size)
                ...
                for step in steps:
                    ...
                    if (step % 100 == 0) or (step == step_count - 1):
                        self.__print(feed_dict, sess, step, step_count, step_loss, test_data, test_writer,
    ...
    @staticmethod
    def weight_variable(name: str, shape: List[int]) -> Variable:
    ...
    @staticmethod
    def bias_variable(name: str, shape: List[int]) -> Variable:
    ...
    def __print(self, feed_dict, sess, step, steps, step_loss, test_data, test_writer, train_summary, train_writer):
        train_accuracy = sess.run(self.accuracy, feed_dict=feed_dict)
        test_data_batch = test_data[0]
        test_label_batch = test_data[1]
        test_feed_dict = {self.x_input: test_data_batch, self.y_input: test_label_batch}
